...
import { DEFAULT_INSTITUTION_COLOR } from '../../institutions/types'
...
function getSessionBackgroundColor(session: Session): string {
  return session.color ?? DEFAULT_INSTITUTION_COLOR
}

function getSessionMeta(session: Session): string {
  const pieces = [
    session.class ? `${session.class.name} ${session.class.classLevel}` : null,
    session.teacher ? `${session.teacher.firstName} ${session.teacher.lastName}` : null,
  ].filter(Boolean)
...
function SessionPill({
  ...
  return (
    <button
      ...
      style={{ backgroundColor: getSessionBackgroundColor(session) }}
      className={`w-full rounded-lg px-2 py-1 text-left text-xs font-semibold text-on-primary shadow-sm transition-opacity hover:opacity-90 ${className}`}
    >
      ...
      <Card className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            className="w-fit rounded-lg border border-border bg-background px-2 py-1.5 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
          />
          {dateFilter ? (
            <Button
              type="button"
