...
export function ClassForm({ mode, schoolClass }: ClassFormProps) {
  ...
  const schema = useMemo(
    () =>
      mode === 'create' ? createClassFormSchema(t) : editClassFormSchema(t),
    [t, mode],
  )
  ...
  const selectsLoading = isInstitutionsPending

  const onSubmit = (values: CreateClassFormValues | EditClassFormValues) => {
    if (mode === 'create') {
      createMutate(values as CreateClassFormValues)
      return
    }

    if (schoolClass) {
      updateMutate({ id: schoolClass.id, data: values as EditClassFormValues })
    }
  }
      ...
      <Card>
        ...
        >
          ...
          <Input
            ...
            maxLength={FIELD_LIMITS.classLevel}
            {...register('classLevel')}
          />
